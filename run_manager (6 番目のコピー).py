...
class ProcessManager:
    ...
    def kill_client(self):
        ...
        # ==============================================================================
        # ★ 追加: 10回ごとの定期リフレッシュでも、OSレベルで確実にゾンビを根絶やしにする
        # ==============================================================================
        print("  [徹底掃除] 古いソフトウェアの残骸を完全に消去中...")
        targets = [
            "awsim_labs.x86_64", 
            "run_scenario.py",
            "component_container", 
        ...
        for target in targets:
            os.system(f"pkill -15 -x {target} > /dev/null 2>&1")
            os.system(f"pkill -15 -f {target} > /dev/null 2>&1")
        time.sleep(2)
        
        # 2段階目: 強制終了（これで確実に次回のNPCが正常に動く）
        for target in targets:
            os.system(f"pkill -9 -x {target} > /dev/null 2>&1")
            os.system(f"pkill -9 -f {target} > /dev/null 2>&1")
            
        print("  [待機] メモリとポートの解放を待っています(3秒)...")
        time.sleep(3)
    ...
    # ==============================================================================
    # ★ 強化されたクリーンアップ: 自爆を防ぎ、確実に関連プロセスを仕留める
    # ==============================================================================
    def cleanup_all(self):
        # ★ 掃除中にCtrl+Cを連打されても無視する（掃除を絶対に終わらせるための防御）
