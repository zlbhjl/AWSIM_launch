...
class ProcessManager:
    def __init__(self):
        # Tupleの2番目を subprocess.Popen オブジェクトとして保持
    ...
    def _kill_process_group(self, proc: subprocess.Popen, name: str):
        ...
        try:
            ...
            # 1. まずは SIGINT (Ctrl+C と同じ) でお行儀よく終了を促す
        ...
        # 2. まだ生きていれば SIGKILL でOSレベルから強制終了する
        ...
        # 3. ゾンビ化を防ぐため、OSから完全にリソースが解放されるのを待つ
        try:
            proc.wait(timeout=3)
    ...
    def execute(self):
        print("=== テスト自動化システム（オリジナル二重ループ＋確実キル版） ===")
        os.makedirs(OUTPUT_DIR, exist_ok=True)
        
        current_sim_idx = 0 

        # ★ あなたの完璧なオリジナルの外側ループ（インフラ管理）
        while current_sim_idx < REPEAT_COUNT:
            ...
            while current_sim_idx < REPEAT_COUNT:
                ...
                if is_timeout:
                    ...
                    self.kill_infra()  # 強化されたキルで確実にプロセスを全滅させる
                    break              # 内側のループを抜け、外側のループ先頭の起動処理へ戻る
                else:
                    print("  [完了] 次のループへ進みます")
                    self.kill_client()
                    current_sim_idx += 1
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n[!] ユーザーによる中断 (Ctrl+C)")
        manager.cleanup_all()
        # 強制終了を確実にするため、少し待ってからPython自身を終了
        time.sleep(1)
        sys.exit(1)
