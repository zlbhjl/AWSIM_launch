...
class TaskQueueActor:
    ...
    def get_status(self):
        return len(self.queue), self.completed_count
...
def load_config():
    ...
    parser.add_argument("--mode", type=str, choices=["explore", "focus"], default="explore", help="Search mode")
...
def main():
    ...
    cluster_manager.start_cluster(scenario_name, run_mode)
    ...
    strategist = ActiveLearningStrategist(scenario_name, cfg, num_candidates=10000, focus_points=focus_points)
    ...
    try:
        while True:
            q_len, completed = ray.get(task_queue.get_status.remote())
            sys.stdout.write(f"\r[Orchestrator] 進行状況: 完了={completed}/{REPEAT_COUNT} | キュー待機={q_len}   ")
            sys.stdout.flush()
